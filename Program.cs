...
public class Programa
{
    ...
    public static void Main(string[] args)
    {
        ...
        .WriteTo
        .File("WorkerEnrollmentManagment.out",Serilog.Events.LogEventLevel.Debug, "{Message:lj}{NewLine}", encoding: Encoding.UTF8)
        .CreateLogger();


        AppLog.ResponseTime = Convert.ToInt16(DateTime.Now.ToString("fff"));
        AppLog.DateTime = DateTime.Now.ToString("yyyyy-MM-dd hh:mm:ss");
    ...
    public static async void ConsumerReceived(object? sender, BasicDeliverEventArgs e)
    {
        ...
        {
            AppLog.Message = "Se proceso el proceso de incripcion de forma exitosa";
    ...
    public static async Task<EnrollmentResponse> ClientWebService(EnrollmentRequest request)
    {
        ...
        try
        {
            var client = new EnrollmentServiceClient(EnrollmentServiceClient.EndpointConfiguration.BasicHttpBinding_IEnrollmentService_soap, "http://localhost:5055/EnrollmentService.asmx");
        ...
        catch(Exception e)
        {
            ...
            ImprimirLog(500, $"Eror en el sistema: {e.Message}", "Error");
        }
    ...
    private static void ImprimirLog(int responseCode, string message, string typeLog)
    {
        ...
        AppLog.ResponseCode = Convert.ToInt16(DateTime.Now.ToString("fff")) - AppLog.ResponseTime;
